...
class BoredomDetector:
    ...
    公式: boredom = eval_param × e^(-t / 600)
    - eval_param 来自 conversation_ended 事件（对话质量评估）
    - t 为对话结束后的经过时间（秒）
    - 随着时间推移无聊水平逐渐降低（残余参与感衰减）

    触发阈值:
    - boredom < 0.30 → emit boredom_trigger: 残余参与感太低，需要主动发起
    - boredom > 0.70 AND impulsiveness > 0.5 → 结束当前主动对话
    """

    # 可配置参数
    DEFAULT_TICK_INTERVAL = 30       # 秒
    DEFAULT_DECAY_HALFLIFE = 600     # e 折半衰期（秒）
    TRIGGER_THRESHOLD = 0.30         # 无聊触发阈值
    END_CONVERSATION_THRESHOLD = 0.70  # 结束对话阈值
    ...
    def _check_thresholds(self) -> None:
        ...
        # 触发阈值：无聊过低 → 需要主动发起
        if boredom < self.TRIGGER_THRESHOLD:
            if self._on_trigger:
        ...
        # 结束对话阈值：无聊过高 + 冲动
        elif boredom > self.END_CONVERSATION_THRESHOLD and self._impulsiveness > 0.5:
